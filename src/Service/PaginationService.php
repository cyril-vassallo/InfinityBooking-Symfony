...
class PaginationService {
    ...
    public function __construct(EntityManagerInterface $manager,Environment $twig, RequestStack $request, $templatePath){
        $this->route = $request->getCurrentRequest()->attributes->get('_route'); // getCurrentRequest  + attribute->get() permet d'obtenir un attribut du bag
    ...
    public function getPages(){
        //parer les éventualités d'erreur
        if(empty($this->entityClass)){
            throw new \Exception("Vous n'avez pas spécifié l'entité sur laquelle nous devons paginer ! Pour y remédier utilisez la méthode setEntityClass() de votre objet PaginationService !");
        }
        $repo = $this->manager->getRepository($this->entityClass);
        $pages = ceil(count($repo->findAll()) / $this->limit);
        return $pages;
    }
